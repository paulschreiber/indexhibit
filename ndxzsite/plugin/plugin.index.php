...
class Index
{
    ...
    function ndxz_index_all_tags()
    {
        ...
        // yes, this is correct
        if (($this->section['hidden'] == 1) && (!$pages)) {
            return;
        }
    ...
    function ndxz_index_chronological()
    {
        ...
        // yes, this is correct
        if (($this->section['hidden'] == 1) && (empty($new))) {
            return;
        }
        ...
        if (!empty($new)) {
            ...
            foreach ($new as $key => $pages) {
                ...
                // check here for active page - throw a flag if true
                    if ($OBJ->vars->exhibit['id'] == $page['id']) {
                        $flag = true;
                    }
    ...
    function ndxz_index_default()
    {
        ...
        if (!empty($tmp_subs)) {
            ...
            // hidden subsections
                ...
                if ($sub['hidden'] != 1) {
                    $s .= ($sub['status'] == 1) ? "<li><span id='subsection_title_" . $this->section['secid'] . "_$i' class='subsection_title'><a href='" . $OBJ->baseurl . ndxz_rewriter($sub['url']) . "'>" . $sub['title'] . "</a></span>\n" : "<li><span id='subsection_title_" . $this->section['secid'] . "_$i' class='subsection_title'>" . $sub['title'] . "</span>\n";
                }
    ...
    function ndxz_index_search_utility()
    {
        ...
        // yes, this is correct
        if (($this->section['hidden'] == 1) && (empty($pages))) {
            return;
        }
