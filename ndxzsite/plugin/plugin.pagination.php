...
class Pagination
{
    function get_prev_next_entries()
    {
        ...
        if ($OBJ->vars->exhibit['section_id'] != 2) {
            return;
        }
        ...
        if ($entries) {
            ...
            if (!empty($next)) {
                $s .= "<span id='newer' style=''><a href='" . BASEURL . "$next[url]' title='Newer'>Previous</a></span> | ";
            }
            if (!empty($previous)) {
                $s .= "<span id='older' style=''><a href='" . BASEURL . "$previous[url]' title='Older'>Next</a></span>";
            }
            $s .= "</div>";
        }
