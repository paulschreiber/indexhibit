...
class Submedia
{
    ...
    function show_tags($id = 0)
    {
        ...
        foreach ($tags as $tag) {
            $tagged[] = "<a href='" . BASEURL . ndxz_rewriter($tag['url']) . "'>" . str_replace('_', ' ', $tag['tag_name']) . "</a>";
        }
    ...
    function array_neighbor($id, $current, $img)
    {
        ...
        foreach ($rsa as $rw) {
            $arr[] = array($rw['media_id'], $rw['media_file']);
        }
        ...
        foreach ($arr as $key => $ck) {
            ...
            if ($ck[0] == $id) {
                $c = true;
                $px = (isset($arr[$key-1][1])) ? $arr[$key-1][1] : '';
                $nn = true;
            }
            if ($c == true) {
                $c = false;
