...
class Exhibit
{
        ...
        foreach ($this->imgs as $do) {
            foreach ($do as $go) {
                ...
                $title = ($OBJ->vars->exhibit['titling'] == 1) ?
                    ($OBJ->vars->exhibit['media_source'] == 0) ? $go['media_title'] : $go['media_title'] : '';
    ...
    function getAssets()
    {
        ...
        elseif ($OBJ->vars->exhibit['operand'] == 1) {
            $OBJ->page->exhibit['dyn_css'][] = $this->defaultCSS();
            ...
            $OBJ->page->exhibit['dyn_css'][] = $this->defaultCSS();
            //$OBJ->page->exhibit['dyn_css'][] = $this->overlayCSS();
    ...
    function makeLink($img, $title = '')
    {
        ...
        elseif ($OBJ->vars->exhibit['operand'] == 3) {
            return "";
        } // overlay
        else {
