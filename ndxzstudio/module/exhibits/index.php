...
var ide = '$go[id]';";
    ...
    public function page_create()
    {
        ...
        if ($go['x'] == 'exhibit') {
            $this->page_create_exhibit();
        }
        if ($go['x'] == 'link') {
            $this->page_create_link();
        }
...
.size-hover { background: #666; }";
    ...
    public function page_view()
    {
        ...
        $body .= ips(
            ...
            'media_title',
            $rs['media_title'],
            "id='media_title' maxlength='255'",
            'text'
        );
        $body .= ips(
            $this->lang->word('image caption'),
            'input',
            'media_caption',
            $rs['media_caption'],
            "id='media_caption' maxlength='255'",
            'text'
        );
...
var ide = '$go[id]';";
    ...
    public function sbmt_unpublish_x()
    {
        ...
        if (($rs['section_top'] == 1) || ($rs['subdir'] == 1)) {
        } else {
            $clean['url'] = '';
        }
    ...
    public function sbmt_upd_jxs()
    {
        ...
        switch ($_POST['x']) {
            ...
            case 'title':
                ...
                // FIX THIS UP!
                if ($clean['title'] == '') {
                    echo "<span style='color: red;'>" . $this->lang->word('error') . "</span>";
                    exit;
                }
