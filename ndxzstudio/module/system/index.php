...
class System extends Router
{
    ...
    public function page_theme()
    {
        ...
        $body .= "<div class='col'>\n";
        $body .= ips(
            $this->lang->word('exhibition name'),
            'input',
            'obj_name',
            $rs['obj_name'],
            "maxlength='50'",
            'text',
            $this->lang->word('required'),
            'req'
        );
    ...
    public function page_statistics()
    {
        ...
        if ($refers) {
            ...
            span("(".$this->lang->word('past 30').")", "class='small-txt'"), "class='toptext' width='75%'");
...
var baseurl = '" . BASEURL . "';";
    ...
    public function page_section()
    {
        ...
        $body .= ips(
            $this->lang->word('section password'),
            'input',
            'sec_pwd',
            $rs['sec_pwd'],
            "maxlength='32'",
    ...
    public function page_formats()
    {
        ...
        if ($go['x'] == 'add') {
            $this->page_extensions_add();
        }
        if ($go['x'] == 'enable') {
            $this->page_format_enable();
        }
        if ($go['x'] == 'disable') {
            $this->page_format_disable();
        }
        if ($go['x'] == 'popedit') {
            $this->page_format_popedit();
        }
        if ($go['x'] == 'edit') {
            $this->page_format_edit();
        }
        if ($go['id'] >= 1) {
            $this->page_extension();
        }
    ...
    public function page_plugins()
    {
        ...
        if ($go['x'] == 'enable') {
            $this->page_extensions_enable();
        }
        if ($go['x'] == 'disable') {
            $this->page_extensions_disable();
        }
        if ($go['x'] == 'edit') {
            $this->page_extensions_edit();
        }
        if ($go['id'] >= 1) {
            $this->page_extension();
        }
        ...
        if ($this->plugins->plugin) {
            foreach ($this->plugins->plugin as $key => $frw) {
                $available[] = $key;
            }
        }
...
var ide = '$go[id]';";
        ...
        if ($rs['media_mime'] == 'url') {
            $body .= ips(
                $this->lang->word('input url'),
                'input',
                'media_file',
                $rs['media_file'],
                "id='media_file' maxlength='255' style='width: 250px; padding: 4px 1px; border: 1px solid #999;'",
                'text'
            );
        }

        $body .= ips(
            $this->lang->word('image title'),
            'input',
            'media_title',
            $rs['media_title'],
            "id='media_title' maxlength='255' style='width: 250px; padding: 4px 1px; border: 1px solid #999;'",
            'text'
        );
...
var ide = '$go[id]';";
    ...
    public function page_folder()
    {
        ...
        if (isset($_POST['ins_files'])) {
            ...
            if (is_array($files)) {
                $body .= show_the_folders_files($files);
            }
    ...
    public function page_prv()
    {
        ...
        if ($OBJ->hook->registered_hook('pre_load')) {
            $OBJ->hook->do_action('pre_load');
        }
    ...
    public function sbmt_upd_tsettings()
    {
        ...
        // get our error messages
            ...
            // update paths
            if ($temp['section_id'] != $_POST['h_secid']) {
                $this->update_tag_paths($_POST['h_secid'], $temp['section_id']);
            }
    ...
    public function page_users_del()
    {
        ...
        if ($go['id'] != 1) {
            $b .= input('del_user', 'submit', null, $this->lang->word('delete'));
        }
    ...
    public function sbmt_send_login()
    {
        ...
        // update password
        if ($rs) {
            $this->db->updateArray(PX.'users', $clean, "ID='$id'");
        }
    ...
    public function sbmt_del_user()
    {
        ...
        if (($go['id'] != 1) || ($go['id'] == $this->access->prefs['ID'])) {
            $this->db->deleteArray(PX.'users', "ID='$go[id]'");
        }
    ...
    public function page_assets()
    {
        ...
        foreach ($t as $key => $te) {
            $li .= li(href($te, "?a=$go[a]&q=assets&edit=/css/$te"));
        }
        $body .= ul($li, "class='ndxz_files'");
        //$body .= "</div>\n\n";
        ...
        foreach ($t as $key => $te) {
            $li .= li(href($te, "?a=$go[a]&q=assets&edit=/js/$te"));
        }
        $body .= ul($li, "class='ndxz_files'");
        $body .= "</div>\n\n";
        ...
        foreach ($t as $key => $te) {
            $li .= li(href($te, "?a=$go[a]&q=assets&edit=/plugin/$te"));
        }
        $body .= ul($li, "class='ndxz_files'");
        $body .= "</div>\n\n";
