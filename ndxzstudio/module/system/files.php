...
function getFiles()
{
    ...
    if (!$rs) {
        ...
        // fake 'mime', actually
            ...
            $edit = span(href(
                "<img src='asset/img/files-edit.gif' />",
                "?a=$go[a]&amp;q=editfile&amp;id=$a[media_id]"
            ), "class='p-action'");
            ...
            $s .= div(
                $use . $edit . span(href($a['media_file'], $url, "target='show'$nodims"), "class='p-name'"),
...
function createFileBox($num)
{
    ...
    for ($i = 0; $i <= $num; $i++) {
        ...
        $s .= div(
            input("media_title[$i]", 'text', "size='20' maxlength='35'", null).'&nbsp;'.
...
{
    if ($line == '') {
        return li(code('&nbsp;', null), "class='tabo'");
    }
