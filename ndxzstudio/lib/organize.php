...
class Organize
{
    ...
    public function exhibit()
    {
        ...
        return li(
            $activity . "<span class='drag-title{$password}{$link}'>" . strip_tags($this->out['title']) .
            $tagged .  '</span>' . span(
                href($OBJ->lang->word('preview'), "?a=$go[a]&amp;q=prv&amp;id=" .
                $this->out['id']) . ' ' . $edit,
    ...
    public function index_default()
    {
        ...
        if ($this->subsections) {
            foreach ($this->subsections as $key => $sub) {
                ...
                $insert = li(
                    $activity . "<span class='{$password}{$link}' id='s" . $this->section['secid'] . "-" . $key . "' style='padding-left: 13px;'>" . strip_tags($sub['title']) .
                    " <strong style='font-weight: normal; font-size: 10px; color: #666;'>&nbsp;&nbsp;$path</strong></span>" . span(
                        href($OBJ->lang->word('preview'), "?a=$go[a]&amp;q=prv&amp;id=" .
                        $sub['id']) . ' ' . $edit,
                        "class='options' style='color: #000;'"
                    ). $homed . $hidden,
