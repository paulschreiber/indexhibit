...
class Core
{
    ...
    */
    public function assign_core()
    {
        foreach (array('vars', 'template', 'access', 'hook', 'abstracts') as $val) {
            ...
            if (!is_object($class)) {
                $this->$class =& load_class($val, true, 'lib');
            }
    ...
    */
    public function load_lang()
    {
        ...
        if (!is_object($class)) {
            $this->$class =& load_class($class, true, 'lang');
        }
        $this->is_loaded[] = $class;
    }
    ...
    */
    public function load_db()
    {
        $class = strtolower('db');
        if (!is_object($class)) {
            $this->$class =& load_class($class, true, 'db');
        }
        $this->is_loaded[] = $class;
    }
    ...
    */
    public function lib_class($class)
    {
        ...
        $class = strtolower($class);
        if (!is_object($class)) {
            $this->$class =& load_class($class, true, 'lib');
        }
        $this->is_loaded[] = $class;
    }
    ...
    */
    public function parse_class($class)
    {
        ...
        if (file_exists(DIRNAME . '/ndxzsite/plugin/plugin.' . $class . '.php')) {
            ...
            if (!is_object('parse')) {
                $this->parse =& load_class($class, true, 'local');
            }
        } else {
            if (!is_object('parse')) {
                $this->parse =& load_class($class, true, 'lib');
            }
        }
    ...
    */
    public function goto_module($class)
    {
        ...
        $class = strtolower($class);
        if (!is_object($class)) {
            $this->$class =& load_class($class, true, 'mod', true);
        }
        $this->is_loaded[] = $class;
    }
    ...
    */
    public function load_collector($a, $oid = 0)
    {
        ...
        $class = strtolower($class);
        if (!is_object($class)) {
            $this->$class =& load_class($class, true, 'collect', true);
        }
        $this->is_loaded[] = $class;
    }
    ...
    */
        ...
        foreach ($arr as $a) {
            foreach ($a as $b) {
                ...
                if (class_exists($b['function'])) {
                    ...
                    if (!is_object($class)) {
                        $this->$class =& load_class($class, true, 'extend', true);
                    }
                    $this->is_loaded[] = $class;
                }
            }
    ...
    public function dyn_class($class)
    {
        $class = strtolower($class);
        if (!is_object($class)) {
            $this->$class =& load_class($class, true, 'local');
        }
        $this->is_loaded[] = $class;
    }
}
