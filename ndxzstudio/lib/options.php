    ...
    public function source()
    {
        ...
        // if it's not a section top we can't allow section
            if ($OBJ->vars->exhibit['section_top'] != 1) {
                if (isset($OBJ->vars->format_params['source']['section'])) {
                    unset($OBJ->vars->format_params['source']['section']);
                }
            }
            
            if (isset($OBJ->vars->format_params['source']['subsection'])) {
                unset($OBJ->vars->format_params['source']['subsection']);
            }
        } // section top exhibit/page
        elseif ($OBJ->vars->exhibit['section_top'] == 1) {
            if (isset($OBJ->vars->format_params['source']['all'])) {
                unset($OBJ->vars->format_params['source']['all']);
            }
            if (isset($OBJ->vars->format_params['source']['subsection'])) {
                unset($OBJ->vars->format_params['source']['subsection']);
            }
